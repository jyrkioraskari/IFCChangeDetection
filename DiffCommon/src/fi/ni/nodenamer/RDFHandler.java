...
public class RDFHandler {
	...
	public OntModel handleRDF(String directory, String filename, String datatype,DatasetStatsVO dsvo) {
		OntModel temp_model=null;
		try {
			if (datatype.equals("IFC"))
				temp_model = readIFC(directory, filename);
		...
		System.out.println("statements:"
				+ temp_model.listStatements().toList().size());
		dsvo.setOrg_statements(temp_model.listStatements().toList().size());
		OntModel  model=filterModel(temp_model);
		System.out.println("filtered statements:"
				+ model.listStatements().toList().size());
		dsvo.setFiltered_statements(model.listStatements().toList().size());
		return model;
	}
